...
import type { BffContext, ActorTable } from "../types.d.ts";
 ...
 */
export async function requireToken(req: Request, ctx: BffContext): Promise<ActorTable> {
  ...
  try {
    ...
    const sessionResponse = await fetch(`${aipBaseUrl}/api/atprotocol/session`, {
      headers: {

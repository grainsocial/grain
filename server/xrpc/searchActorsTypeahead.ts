...
export default defineQuery("social.grain.unspecced.searchActorsTypeahead", async (ctx) => {
  ...
  const grainItems = await resolve<GrainActorProfile>(grainResult.records.map((r: any) => r.uri));

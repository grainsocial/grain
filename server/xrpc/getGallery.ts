...
export default defineQuery("social.grain.unspecced.getGallery", async (ctx) => {
  ...
  // Taken-down, deactivated and deleted accounts are hidden from everyone,
  // matching every feed — a direct deep link must not bypass that.
  const [repos] = (await db.query(
    `SELECT 1 FROM _repos WHERE did = $1 AND status IN ('takendown', 'deactivated', 'deleted') LIMIT 1`,
    [galleryRow.did],
  )) as unknown[];
